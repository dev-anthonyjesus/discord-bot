"""Cog: Shop, Economia Amor, VIPs e Mimos."""
...
from sistema_economia.config import (
    CANAL_LOG_PRIV,
    CANAL_SHOP,
    CARGO_PROTEGIDO,
    VIPS,
...
from sistema_economia.db import (
    get_user,
    save_db,
    load_db,
)
...
<:cash:1492371548122779758> <@&1492358453179472754>  
...
""",
    ...
    embed.set_footer(text="Painel fixo da CandySHOP")
    return embed
...
class MimoSelect(Select):
    def __init__(self, target: nextcord.Member, author: nextcord.Member):
        ...
        options = [
            nextcord.SelectOption(
                label="100",
                description="Não tava merecendo, mas vou enviar",
                value="100",
                emoji=MIMO_EMOJI,
            ),
            nextcord.SelectOption(
                label="300",
                description="Fez algo que gostei hoje",
                value="300",
                emoji=MIMO_EMOJI,
            ),
            nextcord.SelectOption(
                label="500",
                description="Você me faz um bem danado",
                value="500",
                emoji=MIMO_EMOJI,
            ),
            nextcord.SelectOption(
                label="1000",
                description="Obrigado por ser meu porto seguro",
                value="1000",
                emoji=MIMO_EMOJI,
            ),
            nextcord.SelectOption(
                label="10000",
                description="Hoje foi um dia especial!",
                value="10000",
                emoji=MIMO_EMOJI,
            ),
        ]
        ...
        super().__init__(
            placeholder="Escolha a quantidade de mimos",
            min_values=1,
            max_values=1,
            options=options,
        )
    ...
    async def callback(self, interaction: nextcord.Interaction):
        ...
        if self.target.id == self.author.id:
            await interaction.response.send_message(
                "❌ Você não pode enviar mimos para si mesmo.",
                ephemeral=True,
            )
            return
        ...
        await log_priv(
            interaction.guild,
            f"[MIMOS] {interaction.user} enviou {value} mimos para {self.target}",
        )
...
class UserSelect(Select):
        ...
        if not options:
            options = [
                nextcord.SelectOption(
                    ...
                    value="none",
                    description="Não encontrei pessoas para selecionar.",
                )
            ]
    ...
    async def callback(self, interaction: nextcord.Interaction):
        ...
        if self.values[0] == "none":
            await interaction.response.send_message(
                "❌ Não encontrei membros disponíveis.",
                ephemeral=True,
            )
            return
    ...
    def __init__(self):
        options = [
            nextcord.SelectOption(
                label=vip["nome"],
                value=key,
                description=f"{vip['preco']} moedas",
                emoji=MOEDA_EMOJI,
            )
        ...
        super().__init__(
            placeholder="Escolha o cargo para comprar",
            min_values=1,
            max_values=1,
            options=options,
        )
    ...
    async def callback(self, interaction: nextcord.Interaction):
        ...
        if vip["tipo"] == "protegido":
            if user.get("protegido") and user.get("protegido_expira", 0) > agora:
                await interaction.response.send_message(
                    "❌ Você já possui PROTEGIDO ativo.",
                    ephemeral=True,
                )
                return
            ...
            if cargo:
                ...
                await log_priv(
                    interaction.guild,
                    f"[AVISO] Cargo PROTEGIDO não encontrado: {vip['cargo_id']}",
                )
            ...
            await log_priv(
                interaction.guild,
                f"[COMPRA] {interaction.user} comprou PROTEGIDO por {vip['preco']} moedas",
            )

            await interaction.response.send_message(
                "✅ Você comprou **PROTEGIDO** com sucesso.",
                ephemeral=True,
            )
            return
        ...
        if user.get("vip") and user.get("vip_expira", 0) > agora:
            await interaction.response.send_message(
                "❌ Você já possui um VIP/PATROCÍNIO ativo.",
                ephemeral=True,
            )
            return
        ...
        if cargo:
            ...
            await log_priv(
                interaction.guild,
                f"[AVISO] Cargo {vip['nome']} não encontrado: {vip['cargo_id']}",
            )
        ...
        await log_priv(
            interaction.guild,
            f"[COMPRA] {interaction.user} comprou {vip['nome']} por {vip['preco']} moedas",
        )

        await interaction.response.send_message(
            f"✅ Você comprou **{vip['nome']}** com sucesso.",
            ephemeral=True,
        )
...
class ConfirmarConversao(View):
    ...
    async def confirmar(self, button: Button, interaction: nextcord.Interaction):
        ...
        mimos = int(user.get("mimos", 0))
        moedas = (mimos // 100) * 10
        usados = (mimos // 100) * 100

        if usados <= 0:
            await interaction.response.send_message(
                "❌ Você não tem mimos suficientes para converter.",
                ephemeral=True,
            )
            return
        ...
        await log_priv(
            interaction.guild,
            f"[CONVERSÃO] {interaction.user} converteu {usados} mimos em {moedas} moedas",
        )

        await interaction.response.send_message(
            f"✅ Você converteu {usados} mimos em {moedas} moedas.",
            ephemeral=True,
        )
...
class ShopView(View):
    ...
    async def saldo(self, button: Button, interaction: nextcord.Interaction):
        ...
        embed.add_field(
            name="<:moeda:1492371478362980393> Moedas",
            value=f"```{user['moedas']}```",
            inline=True,
        )

        embed.add_field(
            name="<:mimo:1492379169999552563> Mimos",
            value=f"```{user['mimos']}```",
            inline=True,
        )
    ...
    async def converter(self, button: Button, interaction: nextcord.Interaction):
        ...
        embed = nextcord.Embed(
            ...
            description=(
                ...
                "**Taxa:** 100 <:mimo:1492379169999552563> = "
                "10 <:moeda:1492371478362980393>"
            ),
            color=0xFF69B4,
        )
    ...
    async def comprar(self, button: Button, interaction: nextcord.Interaction):
        ...
        await interaction.response.send_message(
            "Escolha o cargo que deseja comprar:",
            view=view,
            ephemeral=True,
        )
    ...
    async def daily(self, button: Button, interaction: nextcord.Interaction):
        ...
        await log_priv(
            interaction.guild,
            f"[DAILY] {interaction.user} recebeu +5 moedas",
        )

        await interaction.response.send_message(
            "🎁 Você recebeu +5 moedas.",
            ephemeral=True,
        )
...
class ShopCog(commands.Cog):
    ...
    async def verificar_vips(self):
        ...
        for user_id, info in list(data.items()):
            ...
            if (
                info.get("vip")
                and info.get("vip_expira", 0) > 0
                and info["vip_expira"] <= agora
            ):
                key = info["vip"]
                vip = VIPS.get(key)
                ...
                await log_priv(
                    guild,
                    f"[EXPIRAÇÃO] {membro} perdeu {vip['nome'] if vip else key}",
                )
            ...
            if (
                info.get("protegido")
                and info.get("protegido_expira", 0) > 0
                and info["protegido_expira"] <= agora
            ):
                ...
                await log_priv(
                    guild,
                    f"[EXPIRAÇÃO] {membro} perdeu PROTEGIDO",
                )
    ...
    async def vip(self, ctx: commands.Context):
        ...
        if icon:
            ...
            embed.set_footer(
                text=(
                    f"{ctx.guild.name} • Status do seu perfil"
                    if ctx.guild
                    else "Status do seu perfil"
                )
            )
        ...
        embed.add_field(
            name="🛡️ Protegido",
            value=protegido_status,
            inline=True,
        )

        embed.add_field(
            name="🛒 Protegido comprado",
            value=protegido_comprado,
            inline=True,
        )

        embed.add_field(
            name="⏳ Protegido expira",
            value=protegido_expira,
            inline=True,
        )
    ...
    async def on_ready(self):
        ...
        try:
            ...
            log.info("[OK] View persistente da shop registrada.")
        except Exception as e:
            log.error(f"[ERRO] Erro ao registrar view da shop: {e}")

        try:
            if not self.verificar_vips.is_running():
                self.verificar_vips.start()
                log.info("[OK] Loop de expiração de VIPs iniciado.")
            else:
                log.info("[OK] Loop de expiração de VIPs já estava rodando.")
        except Exception as e:
            log.error(f"[ERRO] Erro ao iniciar loop de VIPs: {e}")
        ...
        try:
            ...
            log.info("[OK] Painel da shop enviado/atualizado sem spam.")
        except Exception as e:
            log.error(f"[ERRO] Erro no painel da shop: {e}")
